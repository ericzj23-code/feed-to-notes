...
> **项目前身**：`douyin-link-to-obsidian`（v0.1 → v0.10）。**v0.11 起**更名为 `feed-to-notes`，重构了抓取层与输出层的边界，为后续接入其他平台预留接口。
>
> **当前状态**：仅支持抖音。通用化是方向，不是现状——抖音是目前唯一落地并验证过的平台。其他平台规划见 [ROADMAP.md](ROADMAP.md)。

> **定位**：本工具产出的是**内容索引**，不是内容归档。每条笔记包含标题、作者、发布时间、章节大纲（带时间戳）、热门评论、提及标的——足够检索、关联、决定是否回看。**视频正文 / 字幕不抓取**：抖音 PC 版未在 DOM 暴露，且抓取需下载视频文件，违反本项目"不下载 mp4"原则。要看正文，点 `final_url` 回原视频。
...
- **抖音 PC 版 DOM 没暴露**视频正文文案和字幕轨道的稳定 selector。v0.3 曾尝试"自动点字幕按钮 + 等 STT 加载"（`scrapeDouyinPage` 内 `.xgplayer-texttrack` 操作），实际未稳定成功——库内 71 条笔记 `subtitle` 全部为 `null`。
- **真要正文只有两条路**：(A) 下载 mp4 跑 ASR（whisper / 阿里云语音），(B) 抓视频关键帧让多模态模型读图。**两条都违反"不下载 mp4"原则**，所以本工具不做。
- **本工具定位 = 内容索引**：标题 + 作者 + 时间 + 章节大纲 + 热门评论 + 提及标的——足够你决定要不要点 `final_url` 回原视频看正文。
